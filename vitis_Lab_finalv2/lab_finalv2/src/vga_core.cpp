...
/**********************************************************************
 * hangman core methods
 *********************************************************************/
HangmanCore::HangmanCore(uint32_t core_base_addr, int sprite_size) {
   base_addr = core_base_addr;
   size = sprite_size;
}
...
void HangmanCore::wr_mem(int addr, uint32_t color) {
   io_write(base_addr, addr, color);
}

void HangmanCore::bypass(int by) {
   io_write(base_addr, BYPASS_REG, (uint32_t ) by);
}

void HangmanCore::move_xy(int x, int y) {
   io_write(base_addr, X_REG, x);
   io_write(base_addr, Y_REG, y);
   return;
}

void HangmanCore::wr_ctrl(int32_t cmd) {
   io_write(base_addr, SPRITE_CTRL_REG, cmd);
}
